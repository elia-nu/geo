...
export const validateExpenseForm = (formData) => {
  ...
  if (formData.reason && formData.reason.length > 300) {
    errors.reason = "Reason must be less than 300 characters";
  }
...
export const validateIncomeForm = (formData, { isEdit = false } = {}) => {
  ...
  const hasExpected = String(formData.expectedAmount ?? formData.totalProjectAmount ?? "").trim() !== "";

  if (!hasAmount && !hasExpected) {
    errors.amount = "Enter an amount or total expected project amount";
    errors.expectedAmount = "Enter an amount or total expected project amount";
  }
  ...
  if (hasExpected) {
    const expectedErr = validateMoneyAmount(formData.expectedAmount || formData.totalProjectAmount, {
      fieldName: "Total project amount",
      required: true,
      max: 1_000_000_000,
    });
  ...
  if (formData.nextPaymentAmount && String(formData.nextPaymentAmount).trim() !== "") {
    const nextAmountErr = validateMoneyAmount(formData.nextPaymentAmount, {
      fieldName: "Next payment amount",
      required: false,
      allowZero: true,
      max: 1_000_000_000,
    });
    if (nextAmountErr) errors.nextPaymentAmount = nextAmountErr;
  }
  ...
  const nextDateErr = validateDateField(formData.nextPaymentDate, {
    fieldName: "Next payment date",
    required: false,
  });
  if (nextDateErr) errors.nextPaymentDate = nextDateErr;

  if (formData.clientName && formData.clientName.length > 150) {
    errors.clientName = "Client name must be less than 150 characters";
  }
...
export const validateExpectedPaymentForm = (formData) => {
  ...
  const amountErr = validateMoneyAmount(formData.expectedAmount || formData.totalProjectAmount, {
    fieldName: "Total Project Amount / Expected amount",
    required: true,
    max: 1_000_000_000,
  });
