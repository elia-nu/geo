    ...
    for (const project of projects) {
      ...
      if (daysToEnd < 0) {
        const alert = {
          ...
          message: `Project ${project.name} is overdue by ${Math.abs(daysToEnd)} days`,
          priority: "high",
          status: "active",
          relatedEntityId: null,
          relatedEntityType: null,
      ...
      if (project.milestones && project.milestones.length > 0) {
        for (const milestone of project.milestones) {
          ...
          const daysToMilestone = Math.ceil((dueDate - today) / (1000 * 60 * 60 * 24));
          ...
          if (daysToMilestone < 0) {
            const alert = {
              ...
              message: `Milestone "${milestone.title}" for project ${project.name} is overdue by ${Math.abs(daysToMilestone)} days`,
              priority: "high",
              status: "active",
              relatedEntityId: milestone._id,
              relatedEntityType: "milestone",
      ...
      if (percentTimeElapsed > 50 && project.progress < 30) {
        const alert = {
          ...
          message: `Project ${project.name} is ${Math.round(percentTimeElapsed)}% through timeline but only ${project.progress}% complete`,
          priority: "medium",
          status: "active",
          relatedEntityId: null,
          relatedEntityType: null,
